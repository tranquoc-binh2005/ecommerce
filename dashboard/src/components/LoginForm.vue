...
import type { ILoginRequest } from "@/interfaces/auth.interface.ts"
import {useAuth} from "@/hook/auth/useAuth"

const LoginSchema = zod.object({
  email: zod.string().email('Email không hợp lệ').nonempty('Vui lòng nhập email'),
  password: zod.string().min(6, "Mật khẩu phải có tối thiểu 6 kí tự"),
})

const typeLoginSchema = toTypedSchema(LoginSchema)

const { handleSubmit } = useForm({
  validationSchema: typeLoginSchema,
...
const onSubmit = handleSubmit(async (formValues: ILoginRequest) => {
